...
export async function POST(req: NextRequest) {
  try {
    ...
    // capturedStocks가 있으면 캡처 데이터 기반으로 종목 목록 구성
    const useCapturedStocks = capturedStocks && capturedStocks.length > 0;

    const stockList = useCapturedStocks
      ? capturedStocks!
          .map(
            (c, i) =>
              `- c${i}: ${c.stockName}` +
              (c.currentPrice ? ` [현재가: ${c.currentPrice}` : "") +
              (c.returnRate ? `, 수익률: ${c.returnRate}` : "") +
              (c.avgPrice ? `, 평단가: ${c.avgPrice}` : "") +
              (c.evaluationAmount ? `, 평가금액: ${c.evaluationAmount}` : "") +
              (c.quantity ? `, 수량: ${c.quantity}` : "") +
              (c.currentPrice || c.returnRate ? "]" : "")
          )
          .join("\n")
      : stocks.length > 0
      ? stocks
    ...
    // capturedStocks 사용 시 stockId prefix는 c0, c1...
    const stockIdPrefix = useCapturedStocks ? "c" : "s";
...
${useCapturedStocks ? "캡처 이미지에서 추출한 실제 데이터입니다. 현재가/수익률을 stockBriefings에 반영해주세요." : capturesSummary ? "캡처 분석 데이터가 있는 종목은 현재가/수익률을 stockBriefings에 반영해주세요." : ""}
...
다음 JSON 구조로만 응답하세요 (마크다운 없이 순수 JSON):
{
  ...
  "stockBriefings": [
    {
      "stockId": "${stockIdPrefix}0, ${stockIdPrefix}1... 형식",
      "stockName": "종목명",
      "country": "KR 또는 US",
...
- 캡처 데이터 있는 종목은 실제 수익률 기반으로 판단`;
    ...
    // stockId 매핑: capturedStocks면 c0→stockName, 아니면 s0→stock.id
    const stockIdMap: Record<string, string> = {};
    if (useCapturedStocks) {
      capturedStocks!.forEach((c, i) => {
        stockIdMap[`c${i}`] = c.stockName ?? `종목${i}`;
      });
    } else {
      stocks.forEach((s, i) => {
        stockIdMap[`s${i}`] = s.id;
